  ...
  Future<_ClientStats> _getClientStats(
    ...
    try {
      ...
      final products = await productService
          .getClientProductsStream(organizationId, clientId)
          .first;

      int totalProducts = products.length;
      int urgentCount = 0;

      // Contar productos urgentes (si tienes campo de urgencia en el catálogo)
      // Por ahora retornamos 0, ajusta según tu modelo
      if (filters.onlyUrgent) {
        urgentCount = 0; // Implementar lógica de urgencia si es necesario
      }

      return _ClientStats(totalProducts, urgentCount);
    } catch (e) {
      return _ClientStats(0, 0);
    }
  }
}
